...
export default function IntegrationsPage() {
  ...
  // ✅ الحل: تحديث قيمة الـ telegramId فور توفر بيانات الـ merchant بعد الـ Refresh
  useEffect(() => {
    if (merchant?.telegramChatId) {
      setTelegramId(merchant.telegramChatId);
    }
  ...
  return (
    <div className="space-y-8 w-full max-w-7xl animate-fade-in pb-10" dir="rtl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
        ...
        {/* ── العمود الأيمن (رابط المتجر + بيانات المطورين) ── */}
          ...
          <Card className="border-slate-200/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-2xl overflow-hidden">
            ...
            <CardContent className="pt-6 space-y-4">
              <div className="flex flex-col gap-3">
                {/* تم تعديل العرض ليشغل المساحة كاملة للوضوح */}
                <div className="w-full h-12 px-4 rounded-xl border border-slate-200 bg-slate-50 flex items-center text-sm text-slate-700 font-mono font-bold overflow-x-auto whitespace-nowrap scrollbar-thin" dir="ltr">
                  {storeUrl}
                </div>
                <div className="flex gap-2 w-full">
                  <Button 
                    variant="outline" 
                    className={cn("flex-1 h-12 rounded-xl border-slate-200 shadow-sm transition-all font-bold flex items-center justify-center gap-2.5", copied && "bg-emerald-50 text-emerald-600 border-emerald-200 hover:bg-emerald-100 hover:text-emerald-700")} 
          ...
          <Card className="border-slate-200/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-2xl overflow-hidden">
            ...
            <CardContent className="pt-6 space-y-4">
              ...
              <p className="text-xs font-medium text-slate-500 flex items-center gap-1.5">
                ...
                للحصول على الـ Token الخاص بك، يمكنك نسخه من سجلات تسجيل الدخول.
              </p>
            </CardContent>
          </Card>
        </div>

        {/* ── العمود الأيسر (روبوت تيليجرام) ── */}
        <div className="space-y-8">
          <Card className="border-slate-200/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-2xl overflow-hidden">
            <CardHeader className="bg-slate-50/50 border-b border-slate-100 pb-4">
              ...
              <div className="bg-[#0088cc]/5 border border-[#0088cc]/10 rounded-2xl p-5 space-y-3">
                ...
                <ol className="space-y-3 text-sm font-medium text-slate-700">
                  ...
                  <li className="flex items-start gap-2.5">
                    ...
                    <span>انسخ الرقم الذي سيظهر لك وقم بلصقه في الحقل بالأسفل.</span>
                  </li>
                </ol>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-col xl:flex-row gap-3">
                  ...
                  <Button 
                    ...
                    disabled={saving}
                    className="h-12 px-8 rounded-xl bg-[#0088cc] hover:bg-[#0077b3] text-white font-bold shadow-lg shadow-[#0088cc]/20 transition-all flex items-center justify-center gap-2.5"
                  >
                    {saving ? <span>جاري الاتصال...</span> : <><CheckCircle2 className="w-5 h-5" /><span>حفظ وتفعيل</span></>}
                  </Button>
                </div>
              </div>
